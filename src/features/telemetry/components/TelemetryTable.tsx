...
function UploadStatusBadge({ status }: { status: string | undefined }) {
  if (status === "Success") {
    return <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none shadow-none"><span className="mr-1.5 h-1.5 w-1.5 rounded-full bg-emerald-500"></span>Success</Badge>;
  }
  if (status === "Retrying") {
    return <Badge className="bg-amber-100 text-amber-700 hover:bg-amber-100 border-none shadow-none"><span className="mr-1.5 h-1.5 w-1.5 rounded-full bg-amber-500"></span>Retrying</Badge>;
  }
  if (status === "Failed") {
    return <Badge className="bg-rose-100 text-rose-700 hover:bg-rose-100 border-none shadow-none"><span className="mr-1.5 h-1.5 w-1.5 rounded-full bg-rose-500"></span>Failed</Badge>;
  }
  return <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none shadow-none"><span className="mr-1.5 h-1.5 w-1.5 rounded-full bg-emerald-500"></span>Success</Badge>; // Default to success for mocked data if undefined
}
          ...
          <div className="bg-slate-900 rounded-md p-4 overflow-x-auto mt-2">
            <pre className="text-emerald-400 font-mono text-xs leading-relaxed">
              {selectedPayload && JSON.stringify({
                ...
                latitude: 14.6205, // Mocked lat/lng as raw payload examples
                longitude: 121.0048,
